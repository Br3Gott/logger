...
fs.watchFile(path, (eventType, filename) => {
    ...
    readLastLines.read(path, 1)
        .then((line) => {
            ...
            console.log(split[0]);
            console.log({host: split[1], user_agent: split[2], protocol: split[3], resource: split[4],});
            io.emit("log", {host: split[1], user_agent: split[2], protocol: split[3], resource: split[4],});
            ...
            fetch('http://ip-api.com/json/' + split[0])
                ...
                .then(data => {
                    if(data.lat != null) {
                        ...
                        io.emit("coords", {"lat": (parseFloat(data.lat)), "lon": (parseFloat(data.lon))});
                        io.emit("geoip", data);
                    }
                })
